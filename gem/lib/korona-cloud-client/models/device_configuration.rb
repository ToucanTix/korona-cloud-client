...
module KoronaCloudClient
  ...
  class DeviceConfiguration < ApiModelBase
    attr_accessor :port
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KoronaCloudClient::DeviceConfiguration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] device_parameters Value to be assigned
    def device_parameters=(device_parameters)
      if device_parameters.nil?
        fail ArgumentError, 'device_parameters cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] production_type_settings Value to be assigned
    def production_type_settings=(production_type_settings)
      if production_type_settings.nil?
        fail ArgumentError, 'production_type_settings cannot be nil'
      end

      @production_type_settings = production_type_settings
    end
    ...
    def hash
      [port, device, baud_rate, init_command, init_command_variable, port_parameter, port_parameter_variable, default_device, device_parameters, production_type_settings, name].hash
    end
