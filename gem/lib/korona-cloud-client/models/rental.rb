...
module KoronaCloudClient
  # A rental document representing a loan of items to a customer.
  class Rental < ApiModelBase
    ...
    # List of items included in this rental.
    attr_accessor :items
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KoronaCloudClient::Rental` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KoronaCloudClient::Rental`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [active, id, revision, number, booking_time, cashier, comment, customer, order_number, organizational_unit, warehouse, point_of_sale, total, finish_time, items].hash
    end
