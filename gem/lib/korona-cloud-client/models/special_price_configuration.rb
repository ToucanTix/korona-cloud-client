...
module KoronaCloudClient
  # Configuration defining special price validity windows and target organizational units.
  class SpecialPriceConfiguration < ApiModelBase
    ...
    # Name of the special price configuration.
    attr_accessor :name

    # Start date and time when this special price configuration becomes valid. yyyy-MM-dd'T'HH:mm:ssXXX
    attr_accessor :valid_from

    # End date and time until this special price configuration is valid. yyyy-MM-dd'T'HH:mm:ssXXX
    attr_accessor :valid_to

    # Organizational units where this special price configuration applies.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KoronaCloudClient::SpecialPriceConfiguration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KoronaCloudClient::SpecialPriceConfiguration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [active, id, revision, number, name, valid_from, valid_to, organizational_units].hash
    end
