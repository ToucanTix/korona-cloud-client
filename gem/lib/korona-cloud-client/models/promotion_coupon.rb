...
module KoronaCloudClient
  # required if `type`=COUPON_DISCOUNT
  class PromotionCoupon < ApiModelBase
    # Code type.
    ...
    def self.openapi_types
      {
        ...
        :'text' => :'String',
        :'validity' => :'PromotionCouponValidity'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KoronaCloudClient::PromotionCoupon` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KoronaCloudClient::PromotionCoupon`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      code_type_validator = EnumAttributeValidator.new('String', ["BARCODE", "CODEWORD", "QR_CODE"])
    ...
    def code_type=(code_type)
      validator = EnumAttributeValidator.new('String', ["BARCODE", "CODEWORD", "QR_CODE"])
    ...
    def hash
      [code_type, organizational_units, text, validity].hash
    end
